<?php
/**
* @file bigappjson.class.php
* @Brief for json encode
* @author youzu
* @version 1.0.0
* @date 2015-07-07
*/
if(!defined('BIGAPPJSONFILE')){
	define ('BIGAPPJSONFILE', 1);	
}
class BIGAPPJSON {
	const JSON_SLICE = 1;
	const JSON_IN_STR = 2;
		...
		switch (gettype($var)) {
			...
			case 'string':
				if(function_exists('diconv') && strtolower($_G['charset']) != 'utf-8') {
					if(strpos($var, '__DONT_DICONV_TO_UTF8___') !== 0){
						$var = diconv($var, $_G['charset'], 'utf-8');
					}
				}
				if(strpos($var, '__DONT_DICONV_TO_UTF8___') === 0){
					$var = substr($var, strlen('__DONT_DICONV_TO_UTF8___'));
				}
				if(false === $var){
					$var = '';
				}
			...
			case 'array':
				if (is_array($var) && count($var) && (array_keys($var) !== range(0, sizeof($var) - 1))) {
					return '{' .
						   join(',', array_map(array('BIGAPPJSON', 'nameValue'),
											   array_keys($var),
											   array_values($var)))
						   . '}';
				}

				return '[' . join(',', array_map(array('BIGAPPJSON', 'encode'), $var)) . ']';
			case 'object':
