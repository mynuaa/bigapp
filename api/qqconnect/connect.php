...
define("CUR_DISCUZ_ROOT", dirname(__FILE__)."/../../../../../");
chdir(CUR_DISCUZ_ROOT);
...
/////////////////////////////////////
global $_G;
$QQLoginBaseUrl = $_G['siteurl']."connect.php";
$_G['connect']['callback_url'] = $QQLoginBaseUrl.'?mod=login&op=callback';
//$siteurl = substr($_G['siteurl'], 0, -35);
//$_G['siteurl'] = $siteurl;
//echo $qqBaseUrl."<br>$siteurl";
//exit(0);
/////////////////////////////////////
...
if($op == 'init') {
	...
	if(!$_G['setting']['connect']['oauth2']) {
        ...
        echo '<script type="text/javascript">setTimeout("window.location.href=\''.$QQLoginBaseUrl.
             '?platform=qq&openid='. $conopenid .'&oauth_token='. $conuin .
             '&receive=yes&mod=login&op=callback&code=&state' .'\'", 1)</script>';
        exit;
	} else {
        ...
        // 把openid与oauth_token信息拼接到地址url里返给客户端做截取用
        echo '<script type="text/javascript">setTimeout("window.location.href=\''.$QQLoginBaseUrl.
             '?platform=qq&openid='. $conopenid .'&oauth_token='. $conuintoken .
             '&receive=yes&mod=login&op=callback&code=&state' .'\'", 1)</script>';
        exit;
	}
