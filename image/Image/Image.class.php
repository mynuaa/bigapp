...
class Image{
    ...
    public function __construct($type = self::IMAGE_GD, $imgname = null){
        ...
        switch ($type) {
            case self::IMAGE_GD:
				...
				{
					$this->have_handle = true;$this->E('gd_info');
				}
                break;
            case self::IMAGE_IMAGICK:
				...
				{
					$this->have_handle = true;$this->E('Imagick');
				}
                break;
            default:
                $this->E('不支持的图片处理库类型');
        }
		$this->this_dir = dirname(__FILE__);
        /* 引入处理库，实例化图片处理对象 */
	...
	private function E($msg)
	{
		...
		$log_file = $thisdir.'/'.date("Y-m-d")."image.log";
